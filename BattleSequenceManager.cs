    ...
    public class BattleSequenceManager : BattleSequenceManagerBase
    {
        ...
        private int CalcAttackWithWeaponDamage(BattleCharacterBase attacker, BattleCharacterBase target, AttackAttributeType attackAttribute, bool isCritical, List<BattleDamageTextInfo> textInfo)
        {
            ...
            if (damage < 0)
            {
                ...
                statusId = Rom.GameSettings.MaxHPStatusID;
            }
            else if (isCritical)
            {
        ...
        private void Heal(BattleCharacterBase target, Rom.EffectParamSettings effectParamSettings, Dictionary<Guid, string> formulaDic, List<BattleDamageTextInfo> textInfo, Guid attribute, BattleCharacterBase effecter = null, Dictionary<Guid, int> totalHealValueDic = null, Dictionary<Guid, Rom.DamageDrainEffectParam> drainPercentDic = null)
        {
            ...
            foreach (var effectParam in effectParamSettings.GetConsumptionStatusValueChangeEffectParamList())
            {
                ...
                switch (effectParam.ChangeType)
                {
                    ...
                    case Util.StatusChangeType.Percent:
                        {
                            var baseStatus = useSkill ? effecter : target;
            ...
            foreach (var key in effectValueDic.Keys.ToArray())
            {
                ...
                if (info.Consumption && (effecter != null))
                {
                    ...
                    effectValueDic[info.guId] = (int)(effectValue * target.ResistanceAttackAttributePercent(attribute) * target.DamageRate);
                }
            }
        ...
        private void EffectSkill(BattleCharacterBase effecter, Rom.NSkill skill, BattleCharacterBase[] friendEffectTargets, BattleCharacterBase[] enemyEffectTargets, List<BattleDamageTextInfo> textInfo, List<RecoveryStatusInfo> recoveryStatusInfo, out BattleCharacterBase[] friend, out BattleCharacterBase[] enemy)
        {
            ...
            foreach (var target in friendEffectTargets)
            {
                ...
                if (friendEffect.EffectParamSettings.UseOldEffectParam)
                {
                        ...
                        !string.IsNullOrEmpty(friendEffect.hitpointFormula)) && isHealParameter)
                    {
                        ...
                        target.HitPoint += effectValue;
                        target.consumptionStatusValue.AddStatus(Rom.GameSettings.MaxHPStatusID, effectValue);
                        ...
                        AddBattleDamageTextInfo(textInfo, textType, target, effectValue.ToString(), Common.Rom.GameSettings.MaxHPStatusID);
                    ...
                    if ((friendEffect.magicpoint != 0 || friendEffect.magicpointPercent != 0) && isHealParameter)
                    {
                        ...
                        var info = gameSettings.GetCastStatusParamInfo(Rom.GameSettings.MaxMPStatusID, true);
                        ...
                        AddBattleDamageTextInfo(textInfo, textType, target, effectValue.ToString(), Common.Rom.GameSettings.MaxMPStatusID);
                ...
                {
                    Heal(target, friendEffect.EffectParamSettings, friendSkillFormulaDic, textInfo, friendSkillAttribute, effecter, friendTotalSkillValueDic, friendSkillDrainPercentDic);
                ...
                if (target.HitPoint <= 0)
                {
                    ...
                    else if (!target.IsDeadCondition())
                    {
                        ...
                        // Recovered from \
                        target.HitPoint = 1;
                        target.consumptionStatusValue.SetStatus(Rom.GameSettings.MaxHPStatusID, 1);
                    }
                }
                    ...
                    if (!enemyEffect.EffectParamSettings.UseOldEffectParam)
                    {
                        ...
                        foreach (var effectParam in enemyEffect.EffectParamSettings.GetConsumptionStatusValueChangeEffectParamList())
                        {
                            ...
                            switch (effectParam.ChangeType)
                            {
                                ...
                                case Util.StatusChangeType.Direct:
                                    ...
                                    break;
                                case Util.StatusChangeType.Percent:
                                    value = effecter.GetSystemStatus(gameSettings, effectParam.BaseStatusId) * effectParam.ChangeParam / 100;
                                    break;
                                default:
                                    break;
                            }
                        ...
                        foreach (var key in effectValueDic.Keys.ToArray())
                        {
                            ...
                            if (info.Consumption)
                            {
                                // ダメージなので属性耐性を考慮
                                // Since it is damage, consider attribute resistance.
                                effectValueDic[info.guId] = (int)(effectValue * target.ResistanceAttackAttributePercent(enemySkillAttribute) * target.DamageRate);
                            }
                        }
                        ...
                        foreach (var item in enemySkillFormulaDic)
                        {
                            ...
                            effectValueDic[info.guId] += (int)EvalFormula(item.Value, effecter, target, enemySkillAttribute, battleRandom);
                        }
                    ...
                    else
                    {
                            ...
                            !string.IsNullOrEmpty(enemyEffect.hitpointFormula))
                        {
                            ...
                            if (damage >= 0)
                            {
                                ...
                                target.consumptionStatusValue.SubStatus(Rom.GameSettings.MaxHPStatusID, damage);
                            ...
                            AddBattleDamageTextInfo(textInfo, textType, target, Math.Abs(damage).ToString(), Rom.GameSettings.MaxHPStatusID);
                        ...
                        if (enemyEffect.magicpoint != 0 || enemyEffect.magicpointPercent != 0)
                        {
                            ...
                            AddBattleDamageTextInfo(textInfo, textType, target, Math.Abs(damage).ToString(), Rom.GameSettings.MaxMPStatusID);
            ...
            if ((friendSkillDrainPercentDic.Count > 0) || (enemySkillDrainPercentDic.Count > 0))
            {
                ...
                else
                {
                    if (totalHitPointDamage > 0)
                    {
                        ...
                        effecter.consumptionStatusValue.AddStatus(Rom.GameSettings.MaxHPStatusID, totalHitPointDamage);

                        AddBattleDamageTextInfo(textInfo, BattleDamageTextInfo.TextType.Heal, effecter, totalHitPointDamage.ToString(), Rom.GameSettings.MaxHPStatusID);
                    }
                    ...
                    if (totalMagicPointDamage > 0)
                    {
                        ...
                        AddBattleDamageTextInfo(textInfo, BattleDamageTextInfo.TextType.Heal, effecter, totalMagicPointDamage.ToString(), Rom.GameSettings.MaxMPStatusID);
                    }
                }
            }
            ...
            {
                effecter.HitPoint = 0;
                effecter.consumptionStatusValue.SetStatus(Rom.GameSettings.MaxHPStatusID, 0);
            }
                    ...
                    if (battleRandom.Next(100) < condition.SuccessRate * (100 - percent) / 100)
                    {
                        ...
                        if (condition != null)
                        {
                            if ((target.selectedBattleCommandType != BattleCommandType.Skip))
                            {
                                ...
                                {
                                    target.HitPoint = 0;
                                    target.consumptionStatusValue.SetStatus(Rom.GameSettings.MaxHPStatusID, 0);
                                }
                            }
                        }
        ...
        private bool UseItem(Rom.NItem item, BattleCharacterBase target, List<BattleDamageTextInfo> textInfo, List<RecoveryStatusInfo> recoveryStatusInfo)
        {
...

            {
                ...
                if (!item.EffectParamSettings.UseOldEffectParam)
                {
                    ...
                    Heal(target, item.EffectParamSettings, healFormulaDic, textInfo, Guid.Empty);
                ...
                else
                {
                    ...
                    if (healHitPoint > 0)
                    {
                        ...
                        target.consumptionStatusValue.AddStatus(Rom.GameSettings.MaxHPStatusID, healHitPoint);

                        AddBattleDamageTextInfo(textInfo, BattleDamageTextInfo.TextType.Heal, target, healHitPoint.ToString(), Rom.GameSettings.MaxHPStatusID);
                    }
                    ...
                    if (healMagicPoint > 0)
                    {
                        ...
                        AddBattleDamageTextInfo(textInfo, BattleDamageTextInfo.TextType.Heal, target, healMagicPoint.ToString(), Rom.GameSettings.MaxMPStatusID);
                    }
                        ...
                        if (condition != null)
                        {
                            ...
                            if (condition.deadCondition && condition.deadConditionPercent == 0)
                            {
                                ...
                                if (recoveryHitPoint <= 0)
                                {
                                    ...
                                    AddBattleDamageTextInfo(textInfo, BattleDamageTextInfo.TextType.Heal, target, recoveryHitPoint.ToString(), Rom.GameSettings.MaxHPStatusID);

                                    recoveryHitPoint = Math.Min(recoveryHitPoint, target.MaxHitPoint);

                                    target.HitPoint = recoveryHitPoint;
                                    target.consumptionStatusValue.SetStatus(Rom.GameSettings.MaxHPStatusID, recoveryHitPoint);
                                }
                            }
                        }
            ...
            foreach (var enemyMonster in enemyData)
            {
                if ((enemyMonster.HitPoint <= 0 || enemyMonster.IsDeadCondition()) && battleViewer.IsEndMotion(enemyMonster))
                {
                    if (!battleViewer.ContainsFadeOutCharacter(enemyMonster) && enemyMonster.imageAlpha > 0)
                    {
                        ...
                        enemyMonster.consumptionStatusValue.SetStatus(Rom.GameSettings.MaxHPStatusID, 0);
                ...
                foreach (var e in activeCharacter.conditionInfoDic.Values.ToArray())
                {
                    ...
                    if ((condition != null) && condition.slipDamage)
                    {
                        ...
                        if (damage > 0)
                        {
                            ...
                            activeCharacter.consumptionStatusValue.SubStatus(Rom.GameSettings.MaxHPStatusID, damage);
                            ...
                            AddBattleDamageTextInfo(damageText, BattleDamageTextInfo.TextType.Damage, activeCharacter, damage.ToString(), Rom.GameSettings.MaxHPStatusID);
